...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    $deleteStmt = $conn->prepare("DELETE FROM Ticket WHERE ticket_id = ?");
    $deleteStmt->bind_param("i", $_POST['ticket_id']);
    $deleteStmt->execute();
  
    if ($deleteStmt->affected_rows > 0) {
      $errorMessage = "Reservation deleted successfully.";
      $reservationData = null; // Clear data as reservation is deleted
    } else {
...
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" onsubmit="return confirm('Are you sure you want to delete this reservation?');">
  <input type="hidden" name="ticket_id" value="<?php echo htmlspecialchars($reservationData['ticket_id']); ?>">
  <input class="submit-button" type="submit" name="delete" value="Delete Reservation">
</form>
        </form>
        <?php else: ?>
